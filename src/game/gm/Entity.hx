...
class Entity {
	...
	inline function get_onGround()
		return !destroyed && dy == 0 && yr == 1 && level.hasAnyCollision(cx, cy + 1);

	public var recentlyOnGround(get, never):Bool;

	inline function get_recentlyOnGround()
		return isAlive() && (onGround || cd.has("recentlyOnGround"));

	/**
	 * When the entity started falling
	 */
	...
	**/
	...
	function onTouchWall(wallDirection:Int) {}
	...
	function onPreStepY() {
		...
		// Ceiling collision
		if (yr < 0.6 && level.hasWallCollision(cx, cy - 1))
			yr = 0.6;
	}
