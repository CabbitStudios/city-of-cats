...
class Fx extends dn.Process {
	...
	**/
	...
	public function dirt(x:Float, y:Float, strength:Float) {
		for (i in 0...Std.int(strength * 5)) {
			var p = allocTopNormal(getTile(dict.pixel), x + rnd(0, 3, true), y + rnd(0, 3, true));
			p.alpha = rnd(0.4, 1);
			p.colorize(0x52333f);
			p.moveAng(rnd(0, -M.PI), rnd(0.6 , 3));
			p.frict = rnd(0.8, 0.9);
			p.gy = 0.02;
			p.lifeS = rnd(0, 0.3);
		}
	...
	public function laser(x:Float, y:Float, dir:Int) {
		var f = allocTopAdd(getTile(dict.fxLineDir), x, y);
		f.colorizeRandom(0xFF0000, 0xFF9900);
		f.moveAng(M.PIHALF - M.PIHALF * dir, 20);
		f.lifeS = rnd(0, 0.3);
	}
}
